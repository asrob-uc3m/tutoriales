...
- **tree**: Mostrar contenidos de una carpeta en formato árbol. Con `-L` se puede especificar la profundidad (p.ej. `tree -L 2`). Con `-d` se indica que sólo carpetas (p.ej. `tree -d`). Con `-I` se puede excluir un patrón (p.ej. `tree -I build`) o varios patrones (p.ej. `tree -I "build|__pycache__"`).
